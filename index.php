...
$incomplete_threads = $c_mysqli->query("SELECT * FROM `threads` WHERE `num_turns` < 10 ORDER by id DESC LIMIT 10");
$even = true;
...
$completed_threads = $c_mysqli->query("SELECT * FROM `threads` WHERE `num_turns` = 10 ORDER by id DESC LIMIT 10");
$even = true;
while($t = $completed_threads->fetch_object()) {
    ...
    if ($even) {
        ...
        echo '<a class="their-turn" href="showthread.php?id=' . $t->id . '">' . $ft->content . '</a><br />';
    }
    $even = !$even;
}
